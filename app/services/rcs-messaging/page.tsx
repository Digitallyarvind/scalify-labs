...
const WHY_CARDS = [
  ...
  { icon: TrendingUp, title: '3.5x Higher Click Rates', desc: 'Richer messages drive dramatically more engagement than plain SMS', color: 'bg-emerald-100 text-emerald-600' },
...
const COMPARE_ROWS = [
  ...
  { feature: 'Cost per Message', rcs: true, wa: false, rcsLabel: '₹0.16/msg', waLabel: 'Higher cost' },
...
const STEPS = [
  ...
  { icon: Sparkles, num: '02', title: 'Campaign Setup', desc: 'Design rich message templates — cards, carousels, CTA buttons — and load your contact list.' },
...
const FEATURES = [
  'Verified Sender Branding', 'Rich Cards & Carousels', 'CTA Buttons', 'Bulk Campaign Engine',
  'Analytics Dashboard', 'API Integration', 'Rich Media Messaging', 'Campaign Assistance',
]

const FAQ_ITEMS = [
  { q: 'Does RCS work on all phones?', a: 'RCS works on Android devices running Android 5.0+ with Google Messages (the default messaging app). It covers 500M+ Android users in India. iPhones are not supported — those users receive a standard SMS fallback automatically.' },
  { q: 'Do users need to install an app?', a: 'No. RCS messages arrive in the native Android Messages app already installed on the phone. No additional app download required.' },
  { q: 'Can I send images, videos, and files?', a: 'Yes. RCS supports images, videos, GIFs, PDFs, and file attachments up to 100MB. You can also send interactive cards and carousels.' },
  { q: 'Is RCS cheaper than WhatsApp Business?', a: 'Yes. At ₹0.16/message, RCS is significantly more cost-effective than WhatsApp\'s conversation-based pricing (₹0.30–0.85 per conversation). For bulk campaigns above 50,000 messages, the savings are substantial.' },
  { q: 'What is the minimum order quantity?', a: 'The minimum purchase is 50,000 messages. This ensures campaign viability and cost efficiency at ₹0.16 per message (₹8,000 minimum).' },
  { q: 'Can I use my own brand logo and name?', a: 'Yes. After business verification, your brand logo, name, and description appear inside the user\'s chat. This branded sender identity improves trust and engagement significantly.' },
  { q: 'How fast can campaigns start?', a: 'Business verification takes 5–7 business days. Once approved, you can launch campaigns within 24 hours. Scalify Labs handles the entire verification and onboarding process.' },
]

const WHY_SCALIFY = [
  { icon: Zap, title: 'AI-Enabled Campaign Systems', desc: 'Automated sequences with smart triggers and personalisation' },
  { icon: Clock, title: 'Fast 7-Day Onboarding', desc: 'From verification to first campaign in one week' },
  { icon: Globe, title: 'Enterprise Infrastructure', desc: 'Carrier-grade delivery systems built for high-volume campaigns' },
  { icon: BarChart3, title: 'Transparent Analytics', desc: 'Real-time dashboards with delivery, CTR, and conversion data' },
  { icon: Sparkles, title: 'Growth-Focused Execution', desc: 'Campaigns designed to drive enquiries, bookings, and sales' },
  { icon: Shield, title: 'Transparent Flat Pricing', desc: '₹0.16/msg — no hidden fees, no conversation charges' },
]
    ...
    <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-slate-100 px-5 py-4 flex items-center justify-between rounded-t-2xl">
          <div>
            <h3 className="font-bold text-navy text-lg">Enquire About RCS Messaging</h3>
    ...
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div className="space-y-7">
              ...
              <p className="text-slate-600 text-lg leading-relaxed">
                Send verified, interactive, media-rich business messages directly to Android users with images, videos, CTA buttons, and branded messaging experiences.
              </p>
              ...
              <div className="flex flex-wrap gap-3">
                <button onClick={() => setModalOpen(true)} className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-violet-600 text-white font-bold rounded-full shadow-lg shadow-blue-200 hover:opacity-90 hover:-translate-y-0.5 transition-all">
                  Enquire Now <ArrowRight className="w-4 h-4" />
                </button>
                <button onClick={() => setModalOpen(true)} className="flex items-center gap-2 px-6 py-3 border-2 border-blue-600 text-blue-600 font-bold rounded-full hover:bg-blue-50 transition-colors">
                  Buy RCS Credits
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {['Verified Business Messaging', 'Rich Media & CTA Buttons', 'Better Engagement Than SMS', 'Bulk Campaign Ready', 'Real-Time Analytics', 'No App Required'].map(t => (
            ...
            <div className="relative flex justify-center lg:justify-end">
              <div className="relative">
                ...
                <div className="relative bg-slate-900 rounded-[3rem] p-[10px] shadow-[0_40px_80px_rgba(15,23,42,0.35),0_0_0_1px_rgba(255,255,255,0.04)]">
                  <div className="bg-slate-800 rounded-[2.6rem] p-[3px]">
                    <div className="bg-white rounded-[2.4rem] overflow-hidden w-[260px] h-[520px] flex flex-col">
                      ...
                      {/* Chat header */}
                      <div className="flex items-center gap-2.5 px-3 py-2 border-b border-slate-100 shrink-0 bg-white">
                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-violet-600 flex items-center justify-center text-white text-[11px] font-extrabold shrink-0">SL</div>
                        <div>
                          <p className="text-[11px] font-bold text-slate-900 flex items-center gap-1">Scalify Labs <span className="text-blue-500 text-[10px]">✓</span></p>
                        ...
                        <div className="bg-white rounded-2xl shadow border border-slate-100 overflow-hidden max-w-[95%]">
                          <div className="h-[80px] bg-gradient-to-br from-blue-400 via-indigo-500 to-violet-600 flex items-center justify-center relative">
                            <div className="text-white text-center">
                              ...
                              <p className="text-[9px] font-bold">Exclusive Offer</p>
                            </div>
                            <span className="absolute top-1.5 right-2 bg-white/20 backdrop-blur-sm text-white text-[7px] font-bold px-1.5 py-0.5 rounded-full">24HRS LEFT</span>
                          </div>
                ...
                <div className="absolute -top-4 -right-6 bg-white rounded-2xl shadow-xl border border-slate-100 px-3 py-2 flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-[9px] text-slate-500">CTR vs SMS</p>
                    <p className="text-sm font-extrabold text-slate-900">3.5× Higher</p>
                  </div>
                </div>
                ...
                <div className="absolute top-12 -left-8 bg-blue-600 rounded-xl shadow-xl px-2.5 py-1.5 text-white flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] font-bold">Verified Sender</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
            ...
            {[
              ...
              { icon: Sofa, label: 'Home Furnishing' },
            ].map(({ icon: Icon, label }) => (
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="flex items-center justify-center gap-8">
              ...
              <div className="flex flex-col items-center gap-1">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider rotate-0">Upgrade</span>
              </div>
              {/* Rich RCS */}
              <div className="space-y-3">
                ...
                <div className="bg-white rounded-2xl rounded-tl-sm shadow-xl border border-slate-100 overflow-hidden max-w-[165px]">
                  <div className="flex items-center gap-2 px-3 py-2 border-b border-slate-100 bg-gradient-to-r from-blue-50 to-violet-50">
                    ...
                    <div>
                      <p className="text-[9px] font-bold text-slate-900">Scalify Labs ✓</p>
                      <p className="text-[7px] text-blue-600">Verified</p>
                    </div>
                  </div>
                  ...
                  <div className="p-2">
                    <p className="text-[9px] font-bold text-slate-900">20% Off Today</p>
                    <p className="text-[8px] text-slate-500">Limited time deal</p>
                  </div>
      ...
      <section className="py-16 lg:py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {WHY_CARDS.map((card) => (
              <div key={card.title} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-md hover:-translate-y-1 transition-all duration-200 group">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              <p className="text-xs font-bold text-blue-400 uppercase tracking-widest">Cost Advantage</p>
              <h2 className="text-3xl sm:text-4xl font-extrabold text-white leading-tight">Lower Cost.<br />Richer Experience.</h2>
              <p className="text-slate-400 leading-relaxed">RCS offers a scalable communication solution for businesses running bulk promotional campaigns. Unlike conversation-based billing models, RCS messaging enables:</p>
              <ul className="space-y-3">
                {['Predictable campaign costs', 'Interactive customer experiences', 'Direct Android inbox delivery', 'Large-scale outreach at scale'].map(item => (
              ...
              <div className="p-5 bg-white/5 border border-white/10 rounded-2xl inline-block">
                <p className="text-slate-400 text-xs uppercase tracking-wider font-semibold mb-1">Starts from just</p>
                <p className="text-4xl font-black text-white">₹0.16<span className="text-lg font-semibold text-slate-400">/message</span></p>
                <p className="text-slate-500 text-xs mt-1.5">Minimum purchase: 50,000 messages</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { label: 'Avg CTR vs SMS', value: '3.5×', icon: TrendingUp, color: 'text-emerald-400 bg-emerald-400/10' },
                { label: 'Cost per Message', value: '₹0.16', icon: CircleDollarSign, color: 'text-violet-400 bg-violet-400/10' },
                { label: 'Open Rate', value: '75%+', icon: Activity, color: 'text-amber-400 bg-amber-400/10' },
              ].map(stat => (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 relative">
            ...
            {STEPS.map((step, i) => (
              <div key={step.num} className="relative flex flex-col items-center text-center group">
                ...
                {i < STEPS.length - 1 && (
                  <ArrowRight className="hidden lg:block absolute top-9 -right-3 w-4 h-4 text-blue-300" />
                )}
              </div>
            ))}
          </div>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {FEATURES.map(f => (
                  <div key={f} className="flex items-center gap-2.5 p-3 rounded-xl bg-slate-50 border border-slate-100">
                    <CheckCircle className="w-4.5 h-4.5 w-[18px] h-[18px] text-emerald-500 shrink-0" />
                    <span className="text-sm font-semibold text-slate-700">{f}</span>
                  </div>
                ))}
              </div>
              <button onClick={() => setModalOpen(true)} className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-violet-600 text-white font-bold rounded-full shadow-lg shadow-blue-200 hover:opacity-90 transition-opacity">
      ...
      <section className="py-16 lg:py-24 bg-slate-50">
        <div className="max-w-lg mx-auto px-4 sm:px-6 text-center">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-navy mb-8">Simple Bulk Pricing</h2>

          <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden">
            {/* Price header */}
            <div className="bg-gradient-to-br from-blue-600 to-violet-700 px-8 py-10 text-white text-center">
              <p className="text-xs font-bold uppercase tracking-widest opacity-80 mb-2">RCS Bulk Messaging</p>
              <div className="flex items-end justify-center gap-1">
                <span className="text-5xl font-black">₹0.16</span>
                <span className="text-lg font-semibold opacity-80 mb-2">/message</span>
              </div>
              <p className="text-sm opacity-80 mt-2">Minimum order: <strong className="text-white">50,000 messages</strong></p>
              <div className="mt-3 inline-block bg-white/20 backdrop-blur-sm rounded-full px-4 py-1.5 text-sm font-bold">
                = ₹8,000 minimum
              </div>
            </div>
            ...
            <div className="px-8 py-6 space-y-3">
              {['Rich Messaging (images, videos, carousels)', 'Brand Verification Support', 'CTA Buttons (Book, Call, Visit)', 'Real-Time Analytics Dashboard', 'Dedicated Campaign Assistance', 'API Integration Support'].map(f => (
            ...
            <div className="px-8 pb-7 space-y-3">
              <button onClick={() => setModalOpen(true)} className="w-full py-3.5 bg-gradient-to-r from-blue-600 to-violet-600 text-white font-bold rounded-xl hover:opacity-90 transition-opacity">
                Buy RCS Credits
              </button>
              <button onClick={() => setModalOpen(true)} className="w-full py-3.5 border-2 border-slate-200 text-slate-700 font-semibold rounded-xl hover:border-blue-400 hover:text-blue-600 transition-colors text-sm">
      ...
      <section className="py-16 lg:py-24 bg-navy relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-blue-600 rounded-full blur-[200px] opacity-[0.06]" />
        </div>
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <p className="text-xs font-bold text-blue-400 uppercase tracking-widest mb-3">Why Us</p>
            <h2 className="text-3xl sm:text-4xl font-extrabold text-white">Why Businesses Choose Scalify Labs</h2>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {WHY_SCALIFY.map((item) => (
              <div key={item.title} className="bg-white/5 border border-white/10 rounded-2xl p-5 hover:bg-white/8 transition-colors group">
                <div className="w-10 h-10 bg-blue-500/20 rounded-xl flex items-center justify-center mb-4 group-hover:bg-blue-500/30 transition-colors">
      ...
      <section className="py-16 lg:py-24 bg-gradient-to-br from-blue-600 via-indigo-700 to-violet-800 relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="text-white/80 text-lg mb-8">Launch branded RCS campaigns with images, buttons, carousels, and verified business messaging at ₹0.16/message.</p>
          <div className="flex flex-wrap gap-4 justify-center">
            <button onClick={() => setModalOpen(true)} className="flex items-center gap-2 px-7 py-3.5 bg-white text-blue-700 font-bold rounded-full shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all">
              Enquire Now <ArrowRight className="w-4 h-4" />
            </button>
            <button onClick={() => setModalOpen(true)} className="flex items-center gap-2 px-7 py-3.5 border-2 border-white/50 text-white font-bold rounded-full hover:bg-white/10 transition-colors">
              Buy RCS Credits
            </button>
          </div>
          <p className="text-white/50 text-xs mt-6">Minimum order 50,000 messages · Response within 2 hours</p>
        </div>
      </section>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-100 px-4 py-3 flex gap-3 shadow-lg">
        <button onClick={() => setModalOpen(true)} className="flex-1 py-3 bg-gradient-to-r from-blue-600 to-violet-600 text-white text-sm font-bold rounded-xl shadow shadow-blue-200">
          Enquire Now
        </button>
        <button onClick={() => setModalOpen(true)} className="flex-1 py-3 border-2 border-blue-600 text-blue-600 text-sm font-bold rounded-xl">
          Buy Credits
        </button>
      </div>
